...
class TestLoading:
    ...
    def test_attribute_type_union(self):
        ...
        # class SubMapperConfig(SubTransformationConfig):
        #     axes: List[str]
        #     resolution: int
        #     type: str
        ...
        # class SubCyclicConfig(SubTransformationConfig):
        #     axis: str
        #     range: List

        # class CyclicConfig(TransformationConfig):
        #     # TODO: the option here has to be a dict with one key-value pair
        #     # but the key has to be "mapper"
        #     pass

        class NewDatacubeConfig(ConfigModel):
            axis_options: Dict[str, TransformationConfig]

    # def test_nested(self):
    #     axis_list = {"axes" : ["step", "lat", "lon"]}
    #     # axis_list = {'step': NotRequired[Dict], 'lat': NotRequired[Dict], 'lon': NotRequired[Dict]}

    #     class DeeperConfig(ConfigModel):
    #         deeper_axis_config: Dict = {}

    #     class NewDatacubeConfig(ConfigModel):
    #         # axis_config: TypedDict({}, axis_list)
    #         # example_dict = {}
    ...
    #                 setattr(self, f"{item}", item)
    #                 # setattr(self.__class__, f"{item}", Field(default_factory=lambda: item))
    #         # for axis in axis_list:
    #         #     # axis_config[axis] = DeeperConfig
    #         #     # for i, item in enumerate(self.my_list):
    #         #     setattr(self, f"{axis}", axis)

    #         @classmethod
    #         def from_list(cls, axes: List[str]):
    #             data = {"axes": axes}
    #             return cls(**data)

    #     parser = argparse.ArgumentParser(allow_abbrev=False)
    #     axis_list = axis_list["axes"]
    #     model = NewDatacubeConfig.from_list(axes=axis_list)
    #     conflator = Conflator(app_name="polytope", model=model, argparser=parser)
    #     datacube_config = conflator.load()
    #     assert datacube_config.axis_config == {}
